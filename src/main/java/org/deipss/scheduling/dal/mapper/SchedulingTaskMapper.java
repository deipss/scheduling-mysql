...
public interface SchedulingTaskMapper extends BaseMapper<SchedulingTask> {
    ...
    @Update("update scheduling_task set owner_ip = #{ownerIp}, try_lock_cnt=try_lock_cnt+1 " +
            ...
            "and #{nowTime} between start_time and end_time and #{nowTime} > next_start")
    int lock(@Param("lockName") String lockName, @Param("ownerIp") String ownerIp, @Param("nowTime") LocalTime nowTime);
    ...
    int unlock(@Param("lockName") String lockName, @Param("ownerIp") String ownerIp, @Param("nextTime") LocalTime nextTime);


    @Select("select lock_name from scheduling_task where #{nowTime} between start_time and end_time and #{nowTime} > next_start and task_status ='UNLOCK'")
    List<String> scanLockName(@Param("nowTime") LocalTime nowTime);
}